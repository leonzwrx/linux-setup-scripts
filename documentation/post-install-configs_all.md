...
- **[OPTIONAL]** If `fastfetch` isn't pulling hardware info on a custom PC, create a dummy file `/etc/computername.txt` and add this to `fstab`:
 ```bash
/etc/pwr_spec_name /sys/devices/virtual/dmi/id/product_name none bind 0 0
```
